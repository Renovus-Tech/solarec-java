...
public abstract class BaseStaStatisticDao {
	...
	//--- Protected methods ---------------------
	private MapSqlParameterSource createInsertMapSqlParameterSource(StaStatisticVo vo) {
	...
	private MapSqlParameterSource craeteUpdateMapSqlParameterSource(StaStatisticVo vo) {
		return new MapSqlParameterSource()
			.addValue("stat_pro_id", vo.getStatProId())
			.addValue("stat_value", vo.getStatValue())
			.addValue("stat_date_added", vo.getStatDateAdded())
	...
	private MapSqlParameterSource craeteDeleteMapSqlParameterSource(StaStatisticVo vo) {
		return this.createPkMapSqlParameterSource(vo.getCliId(), vo.getStaId(), vo.getStatDate(), vo.getStatTypeId());
	}

	private MapSqlParameterSource createPkMapSqlParameterSource(Integer cliId, Integer staId, java.util.Date statDate, Integer statTypeId) {
		return new MapSqlParameterSource()
	...
	//--- Public methods ------------------------
	public Collection<StaStatisticVo> findAll() { return this.jdbc.query(SQL_SELECT_ALL, StaStatisticRowWrapper.getInstance()); }
	...
	public void insert(StaStatisticVo vo) {
		KeyHolder holder = new GeneratedKeyHolder();
		this.jdbc.update( SQL_INSERT, this.createInsertMapSqlParameterSource(vo), holder, AUTO_INCREMENT_COLUMNS );
	}

	public void update(StaStatisticVo vo) { this.jdbc.update(SQL_UPDATE, this.craeteUpdateMapSqlParameterSource(vo)); }
	public void delete(StaStatisticVo vo) { this.jdbc.update(SQL_DELETE, this.craeteDeleteMapSqlParameterSource(vo)); }

	public void synchronize(StaStatisticVo vo) {
		if (vo == null) return;
		switch (vo.getSyncType()) {
			case StaStatisticVo.SYNC_INSERT: this.insert(vo); break;
			case StaStatisticVo.SYNC_UPDATE: this.update(vo); break;
			case StaStatisticVo.SYNC_DELETE: this.delete(vo); break;
		}
	}
	public void synchronize(Collection<StaStatisticVo> vos) {
		if (vos == null) return;
		for (StaStatisticVo vo : vos) this.synchronize(vo);
}
