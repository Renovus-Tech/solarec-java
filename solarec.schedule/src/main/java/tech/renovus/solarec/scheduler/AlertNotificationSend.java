		...
		if (CollectionUtil.notEmpty(toSend)) {
			...
			for (CliGenAlertVo alertVo : toSend) {
				...
				for (UsersVo usrVo : notify) {
			        ...
			        context.setVariable("alertContgent", this.parserService.parseAlert(alertVo, locale));
					
			        String emailSubject = this.tranaslationService.forLabel(locale, "email.subject.alert");
					String emailontent = this.templateEngine.process("email_alert.html", context);
					...
					try {
						this.emailService.sendSimpleHtmlMessage(usrVo.getUsrEmail(), emailSubject, emailontent);
